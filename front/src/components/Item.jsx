...
const Item = ({task}) => {
  const { _id, title, date, description, iscompleted, isimportant } = task;
  // console.log(title);
      ...
      <div className='flex flex-col justify-between w-full h-full border border-gray-500 rounded-md py-3 px-4'>
        <div className="upper">
          <h2 className='text-xl font-normal mb-3 relative pb-2'>
            <span className='w-full h-[1px] bg-gray-500 absolute bottom-0'></span>{title}
          </h2>
          <p>{description}</p>
        </div>
        <div className="lower">
          ...
          <div className="item-footer flex justify-between">
            <div className="item-footer-left flex gap-x-2">
              <button className='block py-1 px-4 bg-green-400 text-sm text-white rounded-md'>Completed</button>
